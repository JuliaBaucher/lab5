...
aws s3 mb s3://juliabaucher-cv-kb --region eu-north-1
```
...
Create IAM role: `RAGChatLambdaRole`
...
3. **Deploy Code:**
   ```bash
   cd lambda/embedding-builder
   npm install
   zip -r embedding-builder.zip .
...
1. **Update Existing Lambda Function:**
...
Upload the sample knowledge base file:
```bash
aws s3 cp kb/raw/julia-profile.md s3://juliabaucher-cv-kb/kb/raw/julia-profile.md
```
...
```bash
aws s3 ls s3://juliabaucher-cv-kb/kb/embeddings/
```

You should see `julia-profile.json` file created.
...
aws lambda invoke --function-name JuliaBaucher_CV-upload-api --payload '{"test": "data"}' response.json
```
...
- Keep Lambda runtime and dependencies updated
